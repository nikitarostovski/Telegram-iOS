...
private func settingsItems(data: PeerInfoScreenData?, context: AccountContext, presentationData: PresentationData, interaction: PeerInfoInteraction, isExpanded: Bool) -> [(AnyHashable, [PeerInfoScreenItem])] {
    ...
    items[.animation]!.append(PeerInfoScreenDisclosureItem(id: 1, text: "Animations", icon: PresentationResourcesSettings.animations, action: {
        interaction.openSettings(.animations)
    }))
...
private final class PeerInfoScreenNode: ViewControllerTracingNode, UIScrollViewDelegate {
    ...
    fileprivate func openSettings(section: PeerInfoSettingsSection) {
        switch section {
            case .animations:
                let p: PresentationData = (context.sharedContext.currentPresentationData.with { $0 })
                let theme = p.theme

                let vc = ASViewController()
                vc.navigationItem.title = "Animation Settings"
                
                vc.theme.background = theme.list.blocksBackgroundColor
                vc.theme.cellBackground = theme.list.itemBlocksBackgroundColor
                vc.theme.navigationBar = theme.rootController.navigationBar.backgroundColor
                vc.theme.sectionTitleColor = theme.list.sectionHeaderTextColor
                vc.theme.destructiveColor = UIColor.red
                vc.theme.separator = theme.list.itemBlocksSeparatorColor
                vc.theme.tintColor = theme.rootController.navigationBar.accentTextColor
                
                vc.theme.textBlack = theme.list.itemPrimaryTextColor
                vc.theme.textGray = theme.list.itemDisabledTextColor
                
                vc.theme.curveYellow = UIColor(hex: "ffcd00") ?? .yellow
                vc.theme.curveLightGray = theme.list.itemDisabledTextColor.withAlphaComponent(0.25)
                
                vc.theme.shadow = theme.list.blocksBackgroundColor.oppositeColor()
                
                vc.theme.keyboard = theme.rootController.keyboardColor.keyboardAppearance
