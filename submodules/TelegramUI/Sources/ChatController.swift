...
public final class ChatControllerImpl: TelegramBaseController, ChatController, GalleryHiddenMediaTarget, UIDropInteractionDelegate {
    ...
    public init(context: AccountContext, chatLocation: ChatLocation, chatLocationContextHolder: Atomic<ChatLocationContextHolder?> = Atomic<ChatLocationContextHolder?>(value: nil), subject: ChatControllerSubject? = nil, botStart: ChatControllerInitialBotStart? = nil, mode: ChatControllerPresentationMode = .standard(previewing: false), peekData: ChatPeekTimeout? = nil, peerNearbyData: ChatPeerNearbyData? = nil, greetingData: ChatGreetingData? = nil) {
        ...
        self.presentationData = initialPresentationData.withUpdated(wallpaper: effectiveChatWallpaper)
        ...
        self.chatDisplayNode.setupSendActionOnViewUpdate = { [weak self] f in
            self?.chatDisplayNode.historyNode.layoutActionOnViewTransition = { [weak self] transition in
                ...
                if let strongSelf = self, let validLayout = strongSelf.validLayout {
                    ...
                    strongSelf.chatDisplayNode.containerLayoutUpdated(validLayout, navigationBarHeight: strongSelf.navigationHeight, transition: .animated(delay: AnimationManager.shared.yDelay, duration: AnimationManager.shared.yDuration, curve: .custom(AnimationManager.shared.yCurve.0, AnimationManager.shared.yCurve.1, AnimationManager.shared.yCurve.2, AnimationManager.shared.yCurve.3)), listViewTransaction: { updateSizeAndInsets, _, _, _ in
                        ...
                        if isScheduledMessages, let insertedIndex = insertedIndex {
                            ...
                            scrollToItem = ListViewScrollToItem(index: 0, position: .top(0.0), animated: true, curve: .Custom(delay: AnimationManager.shared.yDelay, duration: AnimationManager.shared.yDuration, curve: AnimationManager.shared.yCurve), directionHint: .Up)
                        }
    ...
    private func dismissMediaRecorder(_ action: ChatFinishMediaRecordingAction) {
        if action == .send {
            AnimationManager.shared.tapSource = .mediaButton
