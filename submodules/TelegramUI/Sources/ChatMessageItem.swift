...
//            let nodeClone = (viewClassName as! ChatMessageItemView.Type).init()
            
            node.insertionAnimationOverriden = true
            node.insertionAnimation = { [weak self] completion in
                AnimationManager.shared.animateNode(node: node, item: self, completion: {
                    completion()
                })
                node.insertionAnimationOverriden = false
                AnimationManager.shared.shouldAnimateInsertion = false
            }
            node.setupItem(self)
