...
private final class NativeVideoContentNode: ASDisplayNode, UniversalVideoContentNode {
    ...
    func updateLayout(size: CGSize, transition: ContainedViewLayoutTransition) {
        ...
        if case let .animated(delay, duration, curve) = transition, fromFrame != toFrame, !fromFrame.width.isZero, !fromFrame.height.isZero, !toFrame.width.isZero, !toFrame.height.isZero {
            ...
            self.playerNode.layer.animate(from: NSValue(caTransform3D: transform), to: NSValue(caTransform3D: CATransform3DIdentity), keyPath: "transform", timingFunction: curve.timingFunction, duration: duration, delay: delay)
        } else {
